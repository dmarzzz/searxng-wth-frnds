...
```bash
swf-node --check              # read-only self-test (no server)
```

The identity and config skeleton are auto-created on first daemon
start; `swf-node init` is only needed if you want to write the
skeleton (or rotate the identity) without launching the server.
